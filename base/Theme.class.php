...
class ThemeSupport {
    ...
    public static function brandTitle($value = null) {
        $title = trim((string)($value ?? 'Stargate Wars'));
        return $title !== '' ? $title : 'Stargate Wars';
    }

    public static function brandSubtitle($value = null) {
        $subtitle = trim((string)($value ?? 'Strategic command and empire operations across the Stargate network'));
        return $subtitle !== '' ? $subtitle : 'Strategic command and empire operations across the Stargate network';
    }
}
